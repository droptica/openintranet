...
/**
 * Test channel that always reports a transient (retryable) failure.
 *
 * Drives the worker's retry/backoff path: each send is classified retryable so
 * the worker bumps attempt_count, schedules a retry and re-enqueues.
 */
#[NotificationChannel(
  id: 'flaky_retryable',
  label: new TranslatableMarkup('Flaky retryable'),
  description: new TranslatableMarkup('Always returns a retryable failure; exercises the worker retry path.'),
)]
final class FlakyRetryableChannel extends NotificationChannelBase {
